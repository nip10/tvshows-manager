...
const userController = {
    ...
    async getWatchlist(req, res) {
        ...
        try {
            ...
            const unwatchedEpisodes = await knex.select('episodes.id', 'episodes.title', 'episodes.overview')
            ...
            watchlist = _.map(watchlist, el => ({ tvshowId: el.tvshowId, data: _(el.episodes).groupBy('season').map((items, season) => ({ season, episodes: items })).value() }));
            // Get all different tvshowIds from the unwatched episodes to fetch their posters
            const tvshowIds = _.map(watchlist, 'tvshowId');
            const tvshowPosters = await knex('tvshows').select('images', 'name as tvshowName')
