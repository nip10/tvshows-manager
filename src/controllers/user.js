 ...
 * @returns {undefined}
 */
export async function removeTvshow(req, res) {
  const tvshowId = Number.parseInt(req.params.tvshowId, 10);
  const userId = Number.parseInt(req.user, 10);
  if (!_.isFinite(userId)) {
    return res.status(400).json({ error: ERROR.AUTH.INVALID_ID });
 ...
 * @returns {undefined}
 */
export async function getNumberOfUnwatchedEpisodes(req, res) {
  const userId = Number.parseInt(req.user, 10);
  try {
 ...
 * @returns {undefined}
 */
export async function setEpisodeWatchedStatus(req, res) {
  const userId = Number.parseInt(req.user, 10);
  const tvshowId = Number.parseInt(req.params.tvshowId, 10);
  ...
  if (!_.isFinite(episodeId)) {
    return res.status(500).json({ error: ERROR.EPISODE.INVALID_ID });
  }
  if (!_.isString(setWatched) || (setWatched !== 'true' && setWatched !== 'false')) {
 ...
 * @returns {undefined}
 */
export async function setSeasonWatched(req, res) {
  const userId = Number.parseInt(req.user, 10);
  const tvshowId = Number.parseInt(req.params.tvshowId, 10);
