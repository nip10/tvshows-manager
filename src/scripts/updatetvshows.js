...
async function updateTvshow(tvshowId) {
  ...
  try {
    ...
    for (const episode of requestEpisodes) {
      // knex upsert
      const insertQuery = knex('episodes').insert(episode);
      const updateQuery = knex('episodes').update(episode);
      const query = util.format(
        '%s ON CONFLICT (id) DO UPDATE SET %s',
        insertQuery.toString(),
        updateQuery.toString().substring(22)
      );
      // substring(22) is used to remove "update "episodes" set" which is part of the update query
      // generated by knex on updateQuery. I've hardcoded this because the query is always the same.
      // Originally, this was done with string.replace and a regex but it would crash when a string had the word "set".
      queriesPromises.push(knex.raw(query));
    }
